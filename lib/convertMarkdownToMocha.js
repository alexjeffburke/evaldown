...
try {
  ...
  transpile = function transpile(code) {
    var babelResult = babelCore.transform(
      code,
      extend({}, babelOptions, {
        sourceMaps: false,
        compact: false
...
function makeTryCatchConstruct(exampleNumber, topLevelStatements) {
  ...
  tryCatch[1].body.handler.body.body[0].argument.callee.name =
    'endOfExample' + exampleNumber;
  tryCatch[2].test.arguments[0].name = '__returnValue' + exampleNumber;
  tryCatch[2].consequent.body[0].argument.callee.object.name =
    '__returnValue' + exampleNumber;
  tryCatch[2].consequent.body[0].argument.arguments[1].name =
    'endOfExample' + exampleNumber;
  tryCatch[2].consequent.body[0].argument.arguments[0].body.body[0].argument.callee.name =
    'endOfExample' + exampleNumber;
  tryCatch[2].alternate.body[0].argument.callee.name =
    'endOfExample' + exampleNumber;
  ...
  codeBlocks.forEach(function(codeBlock, i) {
    ...
    if (codeBlock.flags.freshExpect) {
      Array.prototype.push.apply(
        cursor,
        parseFunctionBody(function f(expect) {
...
module.exports = function(mdSrc, fileName) {
  ...
  codeBlocks.forEach(function(codeBlock, i) {
    ...
    /* eslint-enable */
    itExpressionStatement.expression.arguments[0].value =
      'example #' +
      exampleNumber +
      ' (' +
      fileName +
