...
function errorToInfo(e) {
  if (e.name === "FileEvaluationError") {
    const lines = [`snippets with errors:`];
  ...
  async makeOutputForContent(fileContent, format) {
    ...
    switch (this.capture) {
      case "console":
        evalOpts = {
          globals: {
            console: new InspectedConsole(maker, format)
          }
        };
        break;
      case "expect":
    ...
    if (this.fileGlobals) {
      evalOpts.customGlobals = this.fileGlobals;
    }
  ...
  async processFiles() {
    ...
    for (const file of markdownFiles) {
      try {
        ...
        const errorInfo = errorToInfo(e);
        debug('unable to process "%s" with %s: %s', file, e.name, errorInfo);
      }
    }
