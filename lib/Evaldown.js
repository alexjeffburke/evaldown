...
const UnexpectedMarkdown = require("@alexjeffburke/unexpected-markdown");
...
class Evaldown {
  constructor(options) {
    ...
    const wrapper =
      typeof outputWrapper === "function" ? outputWrapper : noopWrapper;
  ...
  async makeOutputForContent(fileContent, format) {
    ...
    switch (this.capture) {
      case "console":
        evalCons = new InspectedConsole(format);
        evalOpts = {
          globals: {
            console: (_, { expect }) => {
              evalCons[consoleSymbols.expect] = expect;
              return evalCons;
            }
          }
        };
