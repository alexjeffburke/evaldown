...
class Evaldown {
  ...
  async _validateFile(sourceFile, emitter, options) {
    ...
    const { error, results } = markdown.validateSnippets({
      // specify HTML when using that reporter or undefined (default)
      format: options.reporter === "html" ? "html" : options._format
    });

    for (const [indexKey, executionResult] of Object.entries(results)) {
      const index = Number(indexKey);
      let displayIndex = index;
      let displayTitle = `${snippets.get(index).lang} evaluation`;
