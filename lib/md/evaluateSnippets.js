...
function prepareCodeForExecution(snippet, { captureOutput }) {
  const { flags } = snippet;

  if (snippet.transpiled) {
    const { transpiled } = snippet;
    if (flags.async || captureOutput) {
...
async function evaluateSnippet(
  snippet,
  { global, baseExpect, captureOutput = false }
) {
  ...
  try {
    ...
    const code = prepareCodeForExecution(snippet, { captureOutput });
...
module.exports = async function evaluateSnippets(snippets, options) {
  options = options || {};
  const { baseExpect, captureOutput } = options;
